        ...
        if (($hModule->getByDirname('xoopsinfo') && '2.0' == mb_substr(XOOPS_VERSION, 6, 3) && mb_substr(XOOPS_VERSION, 10, 2) < 16)
            ...
            foreach ($mimeTypes as $key => $mimeType) {
                ...
                if (0 == $xoopsDB->getRowsNum($result)) {
                    $sql = 'INSERT INTO ' . $xoopsDB->prefix('mimetypes') . ' VALUES ( 0, ' . $xoopsDB->quoteString($mimeType['mime_ext']) . ', ' . $xoopsDB->quoteString($mimeType['mime_types']) . ', ' . $xoopsDB->quoteString($mimeType['mime_name'])
