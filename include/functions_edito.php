...
function edito_uploading_image($filename)
{
    ...
    foreach ($edito_allowed_image as $type_image) {
        if (mb_strstr($type_image, $ext_fileup)) {
...
function edito_uploading_media($filename)
{
    ...
    foreach ($edito_allowed_media as $type_media) {
        if (mb_strstr($type_media, $ext_fileup)) {
...
{
    if (true === $dot) {
        $ext = mb_strtolower(mb_substr($file, mb_strrpos($file, '.')));
    } else {
...
function edito_statmenu($currentoption = 0, $breadcrumb = '')
{
    ...
    if (file_exists(XOOPS_ROOT_PATH . '/modules/edito/language/' . $xoopsConfig['language'] . '/modinfo.php')) {
        require_once XOOPS_ROOT_PATH . '/modules/edito/language/' . $xoopsConfig['language'] . '/modinfo.php';
    } else {
...
function edito_selector(
    ...
    if ('select' == $display || 'box' == $display) {
        ...
        while (list($id, $name, $image, $groups) = $xoopsDB->fetchRow($result)) {
            ...
            if (is_numeric($name) && $name < 10000) {
                $name = constant(mb_strtoupper($db_table . '_' . $db_name . '_' . $name));
            }

            if (is_numeric($name) && $name >= 10000) {
                $name = formatTimestamp($name, 'm');
            }
    ...
    if ('list' == $display) {
        ...
        while (list($id, $name, $image, $groups) = $xoopsDB->fetchRow($result)) {
            ...
            if (is_numeric($name) && $name < 10000) {
                $name = constant(mb_strtoupper($key[0] . '_' . $key[2] . '_' . $name));
            }
    ...
    ';
        ...
        while (list($id, $name, $image, $groups) = $xoopsDB->fetchRow($result)) {
            ...
            if (!is_numeric($image)) {
                $image = '<img src="../uploads/edito/' . $name . '/' . $image . '">';
            } else {
                $image = '';
            }
...
function edito_short_title($title = '', $length = 24, $tiddle = '[...]')
{
    ...
    if (mb_strlen($title) > $length) {
        $title_01 = mb_substr($title, 0, $part1) . $tiddle;

        $title_02 = mb_substr($title, -$part2);
