...
switch ($op) {
    ...
    default:
        ...
        $sql    = ' ( SELECT COUNT(id) FROM ' . $xoopsDB->prefix($xoopsModule->dirname() . '_content') . ' WHERE state = 1)';
        ...
        echo "  <th class='bg3 center' style='width:70px;'><a href='main.php?stat={$stat}&ord=state{$start}'> " . _AM_EDITO_STATE . "</a><br>\n" . "    {$all}<br>{$on}{$hide}{$off}<br>{$html}{$php}{$waiting}\n" . "  </th>\n";
        echo "  <th class='bg3 center bold' style='width:110px;'>" . _AM_EDITO_ACTIONS . "</th>\n";
        echo "</tr>\n";

        // Check edito total
        $sql    = ' SELECT COUNT(id) FROM ' . $xoopsDB->prefix($xoopsModule->dirname() . '_content') . " WHERE state{$state}";
        $result = $xoopsDB->queryF($sql);
        ...
        if ($total > 0) {                // That is, if there ARE editos in the system
            ...
            while (list($id, $subject, $image, $media, $meta, $counter, $state) = $xoopsDB->fetchRow($result)) {
