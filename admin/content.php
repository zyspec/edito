...
function editarticle($id = '', $op = '')
{
    ...
    */
    ...
    if(!isset($image)) { $image = 'blank.gif'; } else { $image = (int)$image; }
    if(!isset($media_url)) { $media_url = ''; } else { $media_url = (int)$media_url; }
    if(!isset($media_file)) { $media_file = ''; } else { $media_file = (int)$media; }
    if(!isset($media_size)) { $media_size = $xoopsModuleConfig['media_size']; } else { $media_size = (int)$media_size; }

    if(!isset($meta_title)) { $meta_title = ''; } else { $meta_title = (int)$meta_title; }
    if(!isset($meta_description)) { $meta_description = $xoopsModuleConfig['moduleMetaDescription']; } else { $meta_description = (int)$meta_description; }
    if(!isset($meta_keywords)) { $meta_keywords = $xoopsModuleConfig['moduleMetaKeywords']; } else { $meta_keywords = (int)$meta_keywords; }

    if(!isset($groups)) { $groups = $xoopsModuleConfig['groups']; } else { $groups = (int)$groups; }

    // Options
    if(!isset($html)) { $html = 1; } else { $html = (int)$html; }
    if(!isset($xcode)) { $xcode = 1; } else { $xcode = (int)$xcode; }
    if(!isset($smiley)) { $smiley = 1; } else { $smiley = (int)$smiley; }
    if(!isset($logo)) { $logo = $xoopsModuleConfig['option_logo']; } else { $logo = (int)$logo; }
    if(!isset($block)) { $block = $xoopsModuleConfig['option_block']; } else { $block = (int)$block; }
    if(!isset($title)) { $title = $xoopsModuleConfig['option_title']; } else { $title = (int)$title; }
    if(!isset($cancomment)) { $cancomment = $xoopsModuleConfig['cancomment']; } else { $cancomment = (int)$cancomment; }
    ...
    $status_radio->addOption("1", _AM_EDITO_WAITING);
    $status_radio->addOption("2", _AM_EDITO_HIDDEN);
    $status_radio->addOption("3", _AM_EDITO_ONLINE);
    $status_radio->addOption("4", _AM_EDITO_HTMLMODE);
    $status_radio->addOption("5", _AM_EDITO_PHPMODE);
    $sform->addElement($status_radio);
...
/*
            $sql = $xoopsDB->queryF( "SELECT COUNT(*)
                   ...
                   if ( $numrows > 0 OR $pid == $id ) {
                        redirect_header( 'index.html', 3, _AM_EDITO_CANTPARENT);
                   }
*/
