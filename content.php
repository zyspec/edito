...
// Module Banner
if (preg_match('/.swf/i', $GLOBALS['xoopsModuleConfig']['index_logo']) && $myrow['state'] >= 3) {
	$banner = '<object
...
} elseif ($GLOBALS['xoopsModuleConfig']['index_logo'] && $myrow['state'] >= 3) {
...
if ('page' == $GLOBALS['xoopsModuleConfig']['media_display'] && $media) {
	...
	if ('image' == $format[1]) {
		...
		$info['displaylogo'] = 1;
        /*                       Generate media logo                            */
		$info['media']       = edito_media($media, $logo, $media_display, $media_options, $myrow['subject'], $GLOBALS['xoopsModuleConfig']['custom_media']);
	}
...
// Admin Links
if (1 == $myrow['state']) {
	$state = "<img src='assets/images/icon/waiting.gif' alt='" . _MD_EDITO_WAITING . "' align='left'>";
	$online = 1;
...
} elseif (3 == $myrow['state']) {
	$state = "<img src='assets/images/icon/online.gif' alt='" . _MD_EDITO_ONLINE . "' align='left'>";
	$online = 1;
	$path = "<a href='.'>" . $GLOBALS['xoopsModule']->getVar('name') . "</a> > " . $subject_org;
	if ('none' != $GLOBALS['xoopsModuleConfig']['navlink_type']) {
        $GLOBALS['xoopsTpl']->assign('navlink', $path);
...
} elseif (4 == $myrow['state']) {
	$state = "<img src='assets/images/icon/html.gif' alt='" . _MD_EDITO_HTML . "' align='left'>";
	$online = 1;
    $GLOBALS['xoopsTpl']->assign('navlink', '');
    $GLOBALS['xoopsTpl']->assign('navlink_type', 'none');
} elseif (5 == $myrow['state']) {
	$state = "<img src='assets/images/icon/php.gif' alt='" . _MD_EDITO_PHP . "' align='left'>";
	$online = 1;
    $GLOBALS['xoopsTpl']->assign('navlink', '');
    $GLOBALS['xoopsTpl']->assign('navlink_type', 'none');
} else {
	$state = "<img src='assets/images/icon/hidden.gif' alt='" . _MD_EDITO_HIDDEN . "' align='left'>";
    $GLOBALS['xoopsTpl']->assign('navlink', '');
    $GLOBALS['xoopsTpl']->assign('navlink_type', 'none');
	$online = 0;
	$path   = '';
...
/*                             Admin links                                 */
/* ----------------------------------------------------------------------- */
if (is_object($GLOBALS['xoopsUser']) && $GLOBALS['xoopsUser']->isAdmin($GLOBALS['xoopsModule']->mid()) && 5 != $myrow['state']) {
	$info['adminlink'] = $state . "<a href='admin/content.php?op=mod&id=$id' title='" . _MD_EDITO_EDIT . "'>
...
} elseif (is_object($GLOBALS['xoopsUser']) && $GLOBALS['xoopsUser']->isAdmin($GLOBALS['xoopsModule']->mid()) && 5 == $myrow['state']) {
	$info['adminlink'] = $state . "<a href='admin/content.php?op=mod&id={$id}'>
    								<img src='assets/images/icon/edit.gif' alt='" . _MD_EDITO_EDIT . "'></a> |
...
$metagen['keywords']    = ('auto' == $GLOBALS['xoopsModuleConfig']['metamanager']) ? $meta_gen : $meta_keywords;
...
if ($metagen['keywords'] || 'manual' == $GLOBALS['xoopsModuleConfig']['metamanager']) {
    if (is_file(XOOPS_ROOT_PATH . '/class/theme.php')) {
